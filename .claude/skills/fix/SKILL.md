...
You are fixing a GitHub issue for the gikiUK/front-end repository.
...
Only create the worktree **after the plan is approved**. The issue number has been extracted above (works whether the argument was a full URL like `https://github.com/gikiUK/front-end/issues/123` or just `123`).
...
After creating the worktree, symlink `node_modules` from the main repo so that JS tooling (prettier, eslint, etc.) and pre-commit hooks work without a full `pnpm install`:

```bash
ln -s /Users/iHiD/Code/giki/front-end/node_modules node_modules
```
...
Run the relevant checks for the changes you made:
...
```bash
cd /Users/iHiD/Code/giki/front-end
